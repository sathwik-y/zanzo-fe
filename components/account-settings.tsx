  ...
  return (
    <section className="mt-8 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5">
      <h2 className="flex items-center gap-2 font-medium">
        <AtSign className="size-4 text-violet-400" /> Instagram account
      </h2>
      {user && (
      ...
      {link?.ig_verified ? (
        ...
        <div className="mt-4 rounded-lg border border-violet-500/30 bg-violet-500/10 p-4 text-sm">
          ...
          <p className="mt-3 select-all rounded-lg bg-zinc-950/60 px-3 py-2 font-mono text-base tracking-widest text-violet-100">
            ZANZO {link.pending_code}
          </p>
          <p className="mt-3 flex items-center gap-2 text-xs text-violet-300/70">
        ...
        <form onSubmit={startLink} className="mt-4 flex flex-wrap items-end gap-3">
          <label className="block grow max-w-xs">
            ...
            <input
              ...
              className="w-full rounded-lg border border-zinc-800 bg-zinc-900/80 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-violet-500/60 focus:outline-none"
            />
          </label>
          <button
            type="submit"
            disabled={busy}
            className="flex items-center gap-2 rounded-lg bg-violet-500/90 px-3 py-2 text-sm font-medium text-white hover:bg-violet-500 disabled:opacity-50"
          >
